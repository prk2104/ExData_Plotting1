...
mydata1 <- transform(mydata1, timestamp=as.POSIXct(paste(Date, Time)))
##, "%d/%m/%Y %H:%M:%S")
...
par(mfrow=c(2,2))
## graph 1
plot(mydata1$timestamp,mydata1$Global_active_power, type="l", xlab="",
     ylab="Global Active Power")
##graph 2
plot(mydata1$timestamp,mydata1$Voltage, type="l", xlab="datetime",
     ylab="Voltage")

## graph 3
plot(mydata1$timestamp,mydata1$Sub_metering_1, type="l", xlab="",
...
##graph 4
plot(mydata1$timestamp,mydata1$Global_reactive_power, type="l",
